...
export default function OrderConfirmationPage({ params }: { params: Promise<{ orderCode: string }> }) {
  const { orderCode } = use(params);
  const [confirmSent, setConfirmSent] = useState(false);
  const { data: resp, isLoading, error, refetch } = useGetOrderByCode(orderCode, {
    query: { refetchInterval: 30_000, queryKey: ['getOrderByCode', orderCode] },
  });
